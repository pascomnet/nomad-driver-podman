		...
		for m := range actorMessageChannel {
			...
			case WaitTaskMsg:
				...
				if ok {
					...
					if err := taskHandle.taskConfig.DecodeDriverConfig(&driverConfig); err != nil {
						logger.Warn("Unable to decode driver config, not starting log streamer", "task", message.TaskID, "error", err)
					} else {
						// start to stream logs if journald log driver is configured and LogCollection is not disabled
						if driverConfig.Logging.Driver == LOG_DRIVER_JOURNALD && !message.DisableLogCollection {
							go taskHandle.runLogStreamer(ctx)
						}
					}
